...
if __name__ == '__main__':
    # Define the values for alpha and n_bits
# alpha_values = [0.000020, 0.000015, 0.000010, 0.000005]
# n_bits_range = range(2, 16)

# # Initialize an array to store average errors for different alpha values
# e_bits_alpha = np.zeros((len(n_bits_range), len(alpha_values)))

# # Vary alpha and n_bits, keeping k_v constant
# for i, n_bits in enumerate(n_bits_range):
#     for j, alpha in enumerate(alpha_values):
#         e_bits_alpha[i, j] = run_sim_ndpcm(n_bits=n_bits, alpha=alpha, enable_plot=0)
...
# # Determine when the reconstructed signal error becomes significantly larger for different alpha values
# for j, alpha in enumerate(alpha_values):
#     for i, n_bits in enumerate(n_bits_range):
...
#     alpha_values = [0.000020, 0.000015, 0.000010, 0.000005]
#     n_bits_range = range(2, 16)
#     k_v = 0.0001  # Define a constant value for k_v
...
#     for i, n_bits in enumerate(n_bits_range):
#         for j, alpha in enumerate(alpha_values):
#             e_bits_alpha[i, j] = run_sim_ndpcm(n_bits=n_bits, alpha=alpha,k_v=k_v, enable_plot=0)

# # Plot average errors versus n_bits for different alpha values
#     plt.title("k_v = 0.0001\nAverage error versus n_bits (varying alpha)")
#     plt.xlabel("n_bits")
#     plt.ylabel("Avg. error")
#     for j in range(len(alpha_values)):
#         plt.plot(n_bits_range, e_bits_alpha[:, j], label="alpha=" + str(alpha_values[j]), marker='o')
...
# Plot average errors versus alpha for different k_v values
    plt.title("n_bits = 6\nAverage error versus alpha (varying k_v)")
    plt.xlabel("Alpha")
    plt.ylabel("Avg. error")
